public class EmployeeBook {
    ...
    public void searchForMinSalary() {
        Employee misSalaryEmployee = employees[0];
        for (int i = 0; i < size; i++) {
            if (misSalaryEmployee.getSalary() > employees[i].getSalary()) {
                misSalaryEmployee = employees[i];
            }
        }
        System.out.println("Минимальная зарплата у:\n" + misSalaryEmployee);
    }

    public void searchForMaxSalary() {
    ...
    public void searchForMinSalaryOfDepartment(int numDepartment) {
        int idEmployee = 0;
        for (int i = 0; i < size; i++) {
        ...
        for (int i = 0; i < size; i++) {
            if (employees[i].getDepartment() == numDepartment && employees[idEmployee].getSalary()
                    > employees[i].getSalary()) {
                idEmployee = i;
            }
        }
        System.out.println("Минимальная зарплата в " + numDepartment + " отделе у\n" + employees[idEmployee]);
    }

    public void searchForMaxSalaryOfDepartment(int numDepartment) {
        double max = 0;
        int idEmployee = 0;
        for (int i = 0; i < size; i++) {
            if (employees[i].getDepartment() == numDepartment) {
    ...
    public void calculatingAmountSalariesOfDepartment(int numDepartment) {
        ...
        System.out.printf("Сумма зарплат в %s отделе %s\n", numDepartment, sum);
    }
    ...
    public void calculationAverageSalaryDepartment(int numDepartment) {
        ...
        System.out.printf("Средняя зарплата в %s отделе равна %s\n", numDepartment, sum);
    }
